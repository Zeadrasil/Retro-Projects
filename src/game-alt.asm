...
.proc nmi_handler
  ...
  CMP #60;accumulator == 60
  BNE not_second_interval; if(previous line)
  INC seconds;increase seconds
  LDA #0;load 0 into accumulator
  STA time;reset time counter
  not_second_interval:;else
  ;Restore Register
...
; now move our ball
 ...
 NOT_HITTOP_A:
 	...
 	bne NOT_HITBOTTOM_A
 		...
 		sta balla_dy
 NOT_HITBOTTOM_A:
 	lda balla_x; get the current x
 	clc
 	adc balla_dx	; add the X velocity
 	sta balla_x
 	cmp #0 ; have we hit the left border
 	bne NOT_HITLEFT_A
 		lda #1 ; reverse direction
 ...
 NOT_HITRIGHT_A:
; now move our ball
 	lda ballb_y; get the current Y
	clc
	adc ballb_dy ; add the Y velocity
 	sta ballb_y; write the change
 	cmp #0 ; have we hit the top border
 	bne NOT_HITTOP_B
 		lda #1 ; reverse direction
 		sta ballb_dy
 NOT_HITTOP_B:
 	...
 	bne NOT_HITBOTTOM_B
 		...
 		sta ballb_dy
 NOT_HITBOTTOM_B:
 	lda ballb_x; get the current x
 	clc
 	adc ballb_dx	; add the X velocity
 	sta ballb_x
 	cmp #0 ; have we hit the left border
 	bne NOT_HITLEFT_B
 		lda #1 ; reverse direction
 ...
 NOT_HITRIGHT_B:
; now move our ball
 	lda ballc_y; get the current Y
	clc
	adc ballc_dy ; add the Y velocity
 	sta ballc_y; write the change
 	cmp #0 ; have we hit the top border
 	bne NOT_HITTOP_C
 		lda #1 ; reverse direction
 		sta ballc_dy
 NOT_HITTOP_C:
 	...
 	bne NOT_HITBOTTOM_C
 		...
 		sta ballc_dy
 NOT_HITBOTTOM_C:
 	lda ballc_x; get the current x
 	clc
 	adc ballc_dx	; add the X velocity
 	sta ballc_x
 	cmp #0 ; have we hit the left border
 	bne NOT_HITLEFT_C
 		lda #1 ; reverse direction
...
LDA balla_y
STA SPRITE_BALLA_ADDR + SPRITE_OFFSET_Y

LDA balla_x
STA SPRITE_BALLA_ADDR + SPRITE_OFFSET_X

LDA ballb_y
STA SPRITE_BALLB_ADDR + SPRITE_OFFSET_Y

LDA ballb_x
STA SPRITE_BALLB_ADDR + SPRITE_OFFSET_X

LDA ballc_y
STA SPRITE_BALLC_ADDR + SPRITE_OFFSET_Y

LDA ballc_x
STA SPRITE_BALLC_ADDR + SPRITE_OFFSET_X
